...
export class PurchaseorderComponent extends ParentPurchaseMgtComponent implements OnInit {
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined, approval: string = "") {
    ...
    this.apiService.getPagination('PurchaseOrder/GetPagedPurchaseOrderList', this.utilService.getQueryString(page, pageCount, query, orderBy, approval, '', 0, this.tranItemCode ?? '')).subscribe(result => {
      this.totalItemsCount = 0;
      this.data = new MatTableDataSource(result.items);
  ...
  public edit(id: number) {
    this.openDialogManage(id, DBOperation.update, this.translate.instant('Create_New_Purchase_Request'), '', AddupdatepurchaseorderComponent);
  }
  public MultipleGrn(id: number) {
    this.apiService.getall(`PurchaseOrder/GRNUpdate/${id}`).subscribe(res => {
      if (res) {
        this.utilService.OkMessage();
        this.refresh();
      }
  ...
  public printing(id: number) {
    this.openDialogManage(id, DBOperation.update, this.translate.instant('Create_New_Purchase_Request'), 'PO', PoprintingpageComponent);
  }

  openIntExpSerial(row: any) {
  ...
  uploadFile(id: any) {
    this.openFileUploadDialogManage(this.translate.instant('Document_Upload'), FileUploadComponent, { module: 'PURMGT', action: 'PO', id: id, sourceId: id });
  }
