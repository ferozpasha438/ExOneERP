...
@Component({
  selector: 'app-poprintingformatthreepage',
  templateUrl: './poprintingformatthreepage.component.html',
  styles: [
  ]
})
export class PoprintingformatthreepageComponent implements OnInit{
  ...
  loadSalesView() {
    this.isLoading = true;
      this.apiService.getall(`PurchaseOrder/getPOPrintingTwoList/${this.data.id}?type=${this.data.from}`).subscribe(res => {
