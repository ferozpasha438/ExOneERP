...
@Component({
  selector: 'app-segmenttwosetup',
  templateUrl: './segmenttwosetup.component.html',
  styles: [
  ]
})
export class SegmenttwosetupComponent extends ParentSystemSetupComponent implements OnInit {
  ...
  loadData() {
    ...
    this.apiService.getall('segmentTwoSetup').subscribe(res => {
      this.list = res;
      this.allList = res;
    });
  }
  ...
  submit() {
    if (this.form.valid) {
      ...
      this.apiService.post('segmentTwoSetup', this.form.value)
        .subscribe(res => {
