...
@Component({
  selector: 'app-costallocationsetup',
  templateUrl: './costallocationsetup.component.html',
  styles: [
  ]
})
export class CostallocationsetupComponent extends ParentSystemSetupComponent implements OnInit {
  ...
  searchFilter(value: string) {
    ...
    if (value.trim() === '') {
      ...
      this.list = this.allList.filter((item) =>
        item.costCode?.toLowerCase().includes(filterValueLower) || item.costType?.toLowerCase().includes(filterValueLower)
        || item.costName?.toLowerCase().includes(filterValueLower)
        || item.costName2?.toLowerCase().includes(filterValueLower)
      );
    }
  }
  ...
  submit() {
    if (this.form.valid) {
      ...
      this.apiService.post('costAllocationSetup', this.form.value)
        .subscribe(res => {
