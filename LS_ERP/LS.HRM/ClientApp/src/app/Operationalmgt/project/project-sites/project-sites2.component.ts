...
@Component({
  selector: 'app-project-sites2',
  templateUrl: './project-sites2.component.html'
})
export class ProjectSites2Component extends ParentOptMgtComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private http: HttpClient, private router: Router, private apiService: ApiService, private authService: AuthorizeService,
    ...
    public pageService: PaginationService, private translate: TranslateService, public dialogRef: MatDialogRef<ProjectSites2Component>) {
    super(authService);
  }
  ...
  ReopenProject(row: any) {
    ...
    let dialogRef = this.oprService.confirmationDialog(this.dialog, ConfirmDialogWindowGeneralComponent);
    (dialogRef.componentInstance as any).titleQuestion = "Are_You_Sure?";
    (dialogRef.componentInstance as any).subTitleQuestion = "Reopen_Project_Cant_Revertable...";
    dialogRef.afterClosed().subscribe(res => {
      if (res == true) {
        row.isExtendProjReq = false;
  ...
  employeePayrollAttendance(row: any) {
    let dialogRef = this.oprService.openAutoHeightWidthDialog(this.dialog, AttendancePayrollReport2Component);
