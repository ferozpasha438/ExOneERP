...
@Component({
  selector: 'app-snd-report-inventory-stock-analysis',
  templateUrl: './snd-report-inventory-stock-analysis.component.html'
})
export class SndReportInventoryStockAnalysisComponent extends ParentSalesMgtComponent implements OnInit {
  ...
  search() {
    ...
    this.reports = [];
    this.pageNumber = 0;
    this.apiService.getall(`sndReports/getInventoryStockAnalysisReport?itemId=${this.itemId}&whCode=${this.whCode}&from=${this.utilService.getCommonDate(this.dateFrom)}&to=${this.utilService.getCommonDate(this.dateTo)}&pageNumber=${this.pageNumber}&pageSize=${this.pageSize}`).subscribe(res => {
  ...
  loadReports(pageNumber: number) {
    ...
    if (pageNumber > Math.floor(this.totalItemsCount / this.pageSize)) {
      ...
      this.apiService.getall(`sndReports/getInventoryStockAnalysisReport?itemId=${this.itemId}&whCode=${this.whCode}&from=${this.utilService.getCommonDate(this.dateFrom)}&to=${this.utilService.getCommonDate(this.dateTo)}&pageNumber=${pageNumber}&pageSize=${this.pageSize}`).subscribe((res: any) => {
