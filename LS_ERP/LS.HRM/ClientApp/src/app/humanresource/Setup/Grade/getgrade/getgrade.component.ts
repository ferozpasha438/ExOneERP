...
import { AddupdategradeComponent } from '../addupdategrade/addupdategrade.component';
...
@Component({
  selector: 'app-getgrade',
  templateUrl: './getgrade.component.html',
  styles: [],
})
export class GetgradeComponent
  ...
  implements OnInit
{
  ...
  public create() {
    this.openDialogManage(
      0,
      DBOperation.create,
      this.translate.instant('AddGrade'),
      'Add'
    );
  }
  public edit(id: number) {
    this.openDialogManage(
      id,
      DBOperation.update,
      this.translate.instant('UpdateGrade'),
      'Update'
    );
  }
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(
      (canDelete) => {
        if (canDelete && id > 0) {
          this.apiService.delete('Grade', id).subscribe((res) => {
            this.refresh();
