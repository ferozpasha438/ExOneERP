...
import { AddupdateholidayComponent } from '../addupdateholiday/addupdateholiday.component';
...
@Component({
  selector: 'app-getholiday',
  templateUrl: './getholiday.component.html',
  
})
export class GetholidayComponent extends ParentHrmAdminComponent implements OnInit {
  ...
  displayedColumns: string[] = ['sno', 'holidayCode', 'holidayNameEn','holidayNameAr','date', 'Actions'];
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    ...
    this.apiService.getPagination('Holiday', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
  ...
  public create() {
    this.openDialogManage(0, DBOperation.create, this.translate.instant('AddHoliday'), 'Add');
  }
  public edit(id: number) {
    this.openDialogManage(id, DBOperation.update, this.translate.instant('UpdateHoliday'), 'Update');
  }
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        this.apiService.delete('Holiday', id).subscribe(res => {
          this.refresh();
