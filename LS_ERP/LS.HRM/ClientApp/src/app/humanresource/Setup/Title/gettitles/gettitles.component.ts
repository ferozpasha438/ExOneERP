...
import { AddupdatetitlesComponent } from '../addupdatetitles/addupdatetitles.component';
import { DeleteConfirmDialogComponent } from 'src/app/sharedcomponent/delete-confirm-dialog';

@Component({
  selector: 'app-gettitles',
  templateUrl: './gettitles.component.html',
  styles: [],
})
export class GettitlesComponent
  ...
  implements OnInit
{
  ...
  public create() {
    this.openDialogManage(
      0,
      DBOperation.create,
      this.translate.instant('AddVisaType'),
      'Add'
    );
  }
  public edit(id: number) {
    this.openDialogManage(
      id,
      DBOperation.update,
      this.translate.instant('UpdateVisaType'),
      'Update'
    );
  }
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(
      (canDelete) => {
        if (canDelete && id > 0) {
          this.apiService.delete('Title', id).subscribe((res) => {
            this.refresh();
