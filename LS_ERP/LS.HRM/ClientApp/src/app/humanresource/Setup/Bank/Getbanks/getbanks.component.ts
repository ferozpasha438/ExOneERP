...
import { AddupdatebanksComponent } from '../addupdatebanks/addupdatebanks.component';

@Component({
  selector: 'app-getbanks',
  templateUrl: './getbanks.component.html',
  styles: [],
})
export class GetbanksComponent
  ...
  implements OnInit
{
  ...
  public create() {
    this.openDialogManage(
      0,
      DBOperation.create,
      this.translate.instant('AddBank'),
      'Add'
    );
  }
  public edit(id: number) {
    this.openDialogManage(
      id,
      DBOperation.update,
      this.translate.instant('UpdateBank'),
      'Update'
    );
  }
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(
      (canDelete) => {
        if (canDelete && id > 0) {
          this.apiService.delete('Bank', id).subscribe((res) => {
            this.refresh();
