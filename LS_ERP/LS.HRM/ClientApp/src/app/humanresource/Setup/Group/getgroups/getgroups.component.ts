...
import { AddupdategroupsComponent } from '../addupdategroups/addupdategroups.component';
...
@Component({
  selector: 'app-getgroups',
  templateUrl: './getgroups.component.html',
  styles: [],
})
export class GetgroupsComponent
  ...
  implements OnInit
{
  ...
  public create() {
    this.openDialogManage(
      0,
      DBOperation.create,
      this.translate.instant('AddGroup'),
      'Add'
    );
  }
  public edit(id: number) {
    this.openDialogManage(
      id,
      DBOperation.update,
      this.translate.instant('UpdateGroup'),
      'Update'
    );
  }
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(
      (canDelete) => {
        if (canDelete && id > 0) {
          this.apiService.delete('Group', id).subscribe((res) => {
            this.refresh();
