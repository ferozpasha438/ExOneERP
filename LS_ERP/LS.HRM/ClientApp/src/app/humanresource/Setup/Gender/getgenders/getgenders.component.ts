...
import { AddupdategendersComponent } from '../addupdategenders/addupdategenders.component';

@Component({
  selector: 'app-getgenders',
  templateUrl: './getgenders.component.html',
  styles: [
  ]
})
export class GetgendersComponent extends ParentHrmAdminComponent implements OnInit {
  ...
  displayedColumns: string[] = ['sno', 'genderCode', 'genderName', 'isActive', 'Actions'];
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    ...
    this.apiService.getPagination('Gender', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
  ...
  private openDialogManage(id: number = 0, dbops: DBOperation, modalTitle: string, modalBtnTitle: string) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdategendersComponent);
  ...
  public create() {
    this.openDialogManage(0, DBOperation.create, this.translate.instant('AddGender'), 'Add');
  }
  public edit(id: number) {
    this.openDialogManage(id, DBOperation.update, this.translate.instant('UpdateGender'), 'Update');
  }
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        this.apiService.delete('Gender', id).subscribe(res => {
          this.refresh();
