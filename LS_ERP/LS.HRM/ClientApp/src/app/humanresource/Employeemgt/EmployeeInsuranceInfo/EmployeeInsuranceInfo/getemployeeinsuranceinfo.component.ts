...
@Component({
  ...
  templateUrl: './getemployeeinsuranceinfo.component.html',
  styles: [
  ]
})
export class GetemployeeinsuranceinfoComponent extends ParentHrmAdminComponent implements OnInit {
  ...
  displayedColumns: string[] = ['sno', 'policyName','policyNumber','policyHolder', 'insuranceTypeName', 'insuranceProviderName', 'insuranceClassName', 'Actions'];
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined, approval: string = "", statusId: string = "", id: number = 0, code: string = "") {
    ...
    this.apiService.getPagination('EmployeeInsurance', this.utilService.getQueryString(page, pageCount, query, orderBy, approval, statusId, id, code)).subscribe(result => {
  ...
  private openDialogManage(id: number = 0, dbops: DBOperation, modalTitle: string, modalBtnTitle: string) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdateinsuranceinfoComponent);
  ...
  public create() {
    this.openDialogManage(0, DBOperation.create, this.translate.instant('AddEmployeeInsurance'), 'Add');
  }
  public edit(id: number) {
    this.openDialogManage(id, DBOperation.update, this.translate.instant('UpdateEmployeeInsurance'), 'Update');
  }
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        this.apiService.delete('EmployeeInsurance', id).subscribe(res => {
          this.refresh();
