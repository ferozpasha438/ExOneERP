...
import { AddupdatedependentinfoComponent } from '../AddupdateDependent/addupdatedependentinfo.component';

@Component({
  selector: 'app-getemployeedependentinfo',
  templateUrl: './getemployeedependentinfo.component.html',
  styles: [
  ]
})
export class GetemployeedependentinfoComponent extends ParentHrmAdminComponent implements OnInit {
  ...
  displayedColumns: string[] = ['sno', 'dependentTypeName', 'dependentName', 'gender', 'idNumber', 'phoneNumber', 'isEmergencyNumber',  'Actions'];
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined, approval: string = "", statusId: string = "", id: number = 0, code: string = "") {
    ...
    this.apiService.getPagination('EmployeeDependent', this.utilService.getQueryString(page, pageCount, query, orderBy,approval, statusId,id,code)).subscribe(result => {
  ...
  private openDialogManage(id: number = 0, dbops: DBOperation, modalTitle: string, modalBtnTitle: string) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdatedependentinfoComponent);
  ...
  public create() {
    this.openDialogManage(0, DBOperation.create, this.translate.instant('AddEmployeeDependent'), 'Add');
  }
  public edit(id: number) {
    this.openDialogManage(id, DBOperation.update, this.translate.instant('UpdateEmployeeDependent'), 'Update');
  }
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        this.apiService.delete('EmployeeDependent', id).subscribe(res => {
          this.refresh();
