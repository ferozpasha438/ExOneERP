...
import { AddupdateemployeeleavetransactionComponent } from '../addupdateemployeeleavetransaction/addupdateemployeeleavetransaction.component';
...
@Component({
  selector: 'app-getemployeeleavetransaction',
  templateUrl: './getemployeeleavetransaction.component.html',
  styles: [
  ]
})
export class GetemployeeleavetransactionComponent extends ParentHrmAdminComponent implements OnInit {
  ...
  private openDialogManage(
    ...
    let dialogRef = this.utilService.openCrudDialog(
      this.dialog,
      AddupdateemployeeleavetransactionComponent
    );
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(
      (canDelete) => {
        if (canDelete && id > 0) {
          this.apiService.delete('EmployeeTransaction', id).subscribe((res) => {
            this.refresh();
