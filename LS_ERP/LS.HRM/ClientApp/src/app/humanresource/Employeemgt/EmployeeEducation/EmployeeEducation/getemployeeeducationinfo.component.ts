...
import { AddupdateeducationinfoComponent } from '../Addupdateeducation/addupdateeducationinfo.component';

@Component({
  selector: 'app-getemployeeeducationinfo',
  templateUrl: './getemployeeeducationinfo.component.html',
  styles: [
  ]
})
export class GetemployeeeducationinfoComponent extends ParentHrmAdminComponent implements OnInit {
  ...
  displayedColumns: string[] = ['sno', 'degreeTypeName', 'isTechnicalQualification', 'qualificationName', 'dateOfCertification', 'collegeOrUniversity', 'courseTypeName',  'Actions'];
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined, approval: string = "", statusId: string = "", id: number = 0, code: string = "") {
    ...
    this.apiService.getPagination('EmployeeQualification', this.utilService.getQueryString(page, pageCount, query, orderBy, approval, statusId, id, code)).subscribe(result => {
  ...
  private openDialogManage(id: number = 0, dbops: DBOperation, modalTitle: string, modalBtnTitle: string) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdateeducationinfoComponent);
  ...
  public create() {
    this.openDialogManage(0, DBOperation.create, this.translate.instant('AddEmployeeEducation'), 'Add');
  }
  public edit(id: number) {
    this.openDialogManage(id, DBOperation.update, this.translate.instant('UpdateEmployeeEducation'), 'Update');
  }
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        this.apiService.delete('EmployeeQualification', id).subscribe(res => {
          this.refresh();
