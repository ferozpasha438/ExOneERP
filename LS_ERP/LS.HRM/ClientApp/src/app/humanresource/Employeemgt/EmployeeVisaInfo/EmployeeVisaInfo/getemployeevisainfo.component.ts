...
import { AddupdatevisainfoComponent } from '../AddupdateVisa/addupdatevisainfo.component';

@Component({
  selector: 'app-getemployeevisainfo',
  templateUrl: './getemployeevisainfo.component.html',
  styles: [
  ]
})
export class GetemployeevisainfoComponent extends ParentHrmAdminComponent implements OnInit {
  ...
  displayedColumns: string[] = ['sno', 'countryName', 'visaTypeName', 'visaNumber', 'isVisaValid', 'Actions'];
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined, approval: string = "", statusId: string = "", id: number = 0, code: string = "") {
    ...
    this.apiService.getPagination('EmployeeVisa', this.utilService.getQueryString(page, pageCount, query, orderBy, approval, statusId, id, code)).subscribe(result => {
  ...
  private openDialogManage(id: number = 0, dbops: DBOperation, modalTitle: string, modalBtnTitle: string) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdatevisainfoComponent);
  ...
  public create() {
    this.openDialogManage(0, DBOperation.create, this.translate.instant('AddEmployeeVisa'), 'Add');
  }
  public edit(id: number) {
    this.openDialogManage(id, DBOperation.update, this.translate.instant('UpdateEmployeeVisa'), 'Update');
  }
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        this.apiService.delete('EmployeeVisa', id).subscribe(res => {
          this.refresh();
