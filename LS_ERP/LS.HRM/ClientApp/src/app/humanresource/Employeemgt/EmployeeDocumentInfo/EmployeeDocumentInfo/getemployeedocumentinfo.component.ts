...
export class GetemployeedocumentinfoComponent extends ParentHrmAdminComponent implements OnInit {
  ...
  displayedColumns: string[] = ['sno', 'documentTypeName', 'name', 'documentNumber', 'isVerified', 'Actions'];
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined, approval: string = "", statusId: string = "", id: number = 0, code: string = "") {
    ...
    this.apiService.getPagination('EmployeeDocument', this.utilService.getQueryString(page, pageCount, query, orderBy, approval, statusId, id, code)).subscribe(result => {
  ...
  private openDialogManage(id: number = 0, dbops: DBOperation, modalTitle: string, modalBtnTitle: string) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdatedocumentinfoComponent, '80%', '55%');
  ...
  public create() {
    this.openDialogManage(0, DBOperation.create, this.translate.instant('AddEmployeeDocument'), 'Add');
  }
  public edit(id: number) {
    this.openDialogManage(id, DBOperation.update, this.translate.instant('UpdateEmployeeDocument'), 'Update');
  }
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        let queryParam = `id=${encodeURIComponent("" + id)}&employeeID=${encodeURIComponent("" + Number(this.employeeNumber))}&deleteDocument=true`;
        this.apiService.getQueryString(`EmployeeDocument/GetEmployeeDocumentById?`, queryParam)
        .subscribe(res => 
          {
            this.refresh();
