...
@Component({
  selector: 'app-addupdatebankvoucher',
  templateUrl: './addupdatebankvoucher.component.html',
  styles: [
  ]
})
export class AddupdatebankvoucherComponent implements OnInit {
  form: FormGroup;
  id: number = 0;
  ...
  constructor(private fb: FormBuilder, private apiService: ApiService,
    private authService: AuthorizeService, private utilService: UtilityService, public dialogRef: MatDialogRef<AddupdatebankvoucherComponent>,
  ...
  setEditForm() {
    this.apiService.get('bankVoucher', this.id).subscribe(res => {
      if (res) {
  ...
  submit() {
    ...
    if (this.form.valid) {
      ...
      this.apiService.post('bankVoucher', this.form.value)
        .subscribe(res => {
