...
@Component({
  selector: 'app-addupdatejournalvoucher',
  templateUrl: './addupdatejournalvoucher.component.html',
  styles: [
  ]
})
export class AddupdatejournalvoucherComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private apiService: ApiService,
    private authService: AuthorizeService, private utilService: UtilityService, public dialogRef: MatDialogRef<AddupdatejournalvoucherComponent>,
  ...
  setEditForm() {
    this.apiService.get('journalVoucher', this.id).subscribe(res => {
      if (res) {
  ...
  addItem() {
    if ((this.drAmount > 0 || this.crAmount > 0) && this.utilService.hasValue(this.branchCode) && this.utilService.hasValue(this.finAcCode)) {
  ...
  submit() {
    ...
    if (!this.hasBranchHidden) {
      ...
      this.apiService.post('journalVoucher', this.form.value)
        .subscribe(res => {
