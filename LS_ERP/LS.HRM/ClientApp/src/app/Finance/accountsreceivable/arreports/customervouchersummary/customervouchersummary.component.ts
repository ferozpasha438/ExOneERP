...
@Component({
  selector: 'app-customervouchersummary',
  templateUrl: './customervouchersummary.component.html',
  styles: [
  ]
})
export class CustomervouchersummaryComponent extends ParentFinMgtComponent implements OnInit {
  ...
  search(page: number = this.pageService.page, pageCount: number = this.pageService.pageCount) {
    ...
    this.apiService.getall(`report/getCustomerVoucherSummaryList?page=${page}&pageCount=${pageCount}&isAllBranches=${this.isAllBranches}&type=${this.type}&custCode=${this.codeControl.value}&from=${this.utilService.getCommonDate(this.dateFrom)}&to=${this.utilService.getCommonDate(this.dateTo)}`).subscribe(res => {
