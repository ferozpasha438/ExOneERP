...
@Component({
  selector: 'app-arpayment',
  templateUrl: './arpayment.component.html',
  styles: [
  ]
})
export class ArpaymentComponent extends ParentFinMgtComponent implements OnInit {
    ...
    if (!this.utilService.hasPaymenRoute()) {
      this.apiService.getall("customerPayment/checkOpenItemMenthod").subscribe(res => {
        if (res) {
          ...
          this.router.navigateByUrl('dashboard/fin/opmarpayment');
        }
        else
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    ...
    this.apiService.getPagination('customerPayment', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  public create() {
    this.openDialogManage(0, DBOperation.create, this.translate.instant('Create_New_Sales_Invoice'), '', AddupdatecustomerpaymentComponent);
  }

  public edit(id: number) {
    this.openDialogManage(id, DBOperation.update, this.translate.instant('Create_New_Sales_Invoice'), '', AddupdatecustomerpaymentComponent);
  }
  ...
  public customerReceipt(id: number) {
    this.openDialogManage(id, DBOperation.update, this.translate.instant('Create_New_Sales_Invoice'), '', Customerreceiptvoucher);
  ...
  public post(id: number, customerCode: string) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        ...
        this.apiService.post('customerPayment/customerPaymentApproval', { customerCode: customerCode, Id: id })
          .subscribe(res => {
            this.initialLoading();
            this.utilService.OkMessage();
          },
