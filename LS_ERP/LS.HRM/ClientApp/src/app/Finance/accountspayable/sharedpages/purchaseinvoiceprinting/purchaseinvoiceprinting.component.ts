...
@Component({
  selector: 'app-purchaseinvoiceprinting',
  templateUrl: './purchaseinvoiceprinting.component.html',
  styles: [
  ]
})
export class PurchaseinvoiceprintingComponent implements OnInit {
  ...
  constructor(private apiService: ApiService,
    ...
    private notifyService: NotificationService, private validationService: ValidationService, public dialogRef: MatDialogRef<PurchaseinvoiceprintingComponent>) {
  ...
  search() {
    ...
    if (this.dateFrom && this.dateTo) {
      ...
      this.apiService.getall(`generateApInvoice/getPurchaseInvoicePrintingList?id=${this.customerId ?? 0}&branchCode=${this.branchCodeControl.value ?? ''}&from=${this.utilService.getCommonDate(this.dateFrom)}&to=${this.utilService.getCommonDate(this.dateTo)}`).subscribe(res => {
