...
@Component({
  selector: 'app-addupdatevendorpayment',
  templateUrl: './addupdatevendorpayment.component.html',
  styles: [
  ]
})
export class AddupdatevendorpaymentComponent implements OnInit {
  form: FormGroup;
  id: number = 0;
  ...
  constructor(private fb: FormBuilder, private apiService: ApiService,
    private authService: AuthorizeService, private utilService: UtilityService, public dialogRef: MatDialogRef<AddupdatevendorpaymentComponent>,
  ...
  setForm() {
    ...
    this.form = this.fb.group({
      ...
      'payType': ['', Validators.required],
      'amount': ['', Validators.required],// this.validationService.numberValidator])],
  ...
  setEditForm() {
    this.apiService.get('vendorPayment', this.id).subscribe(res => {
      if (res) {
  ...
  loadProductdata(event: any) {
    ...
    if (custCode) {
      ...
      this.apiService.getall(`vendorPayment/getVendorToBePaidAmount?customerCode=${custCode}`).subscribe(res => {
        if (res) {
  ...
  submit() {
    if (this.form.valid) {
      ...
      this.apiService.post('vendorPayment', this.form.value)
        .subscribe(res => {
  ...
  reset() {
    this.form.controls['vendCode'].setValue('');
