...
import { ReportPaginationService } from '../../../../sharedcomponent/pagination.service';
  ...
  @Component({
    selector: 'app-opmvendorsummary',
    templateUrl: './opmvendorsummary.component.html',
    styles: [
    ]
  })
  export class OpmvendorsummaryComponent extends ParentFinMgtComponent implements OnInit {
  ...
  filter(val: string): Observable<Array<CustomSelectListItem>> {
    ...
    this.isLoading = true;
      ...
      this.apiService.getall(`report/getOpmVendorSummaryList?page=${page}&pageCount=${pageCount}&isAllCustomers=${this.isAllCustomers}&type=${this.type}&custCode=${this.codeControl.value}&from=${this.utilService.getCommonDate(this.dateFrom)}&to=${this.utilService.getCommonDate(this.dateTo)}`).subscribe(res => {
