...
@Component({
  selector: 'app-vendorpaymentvouchersummary',
  templateUrl: './vendorpaymentvouchersummary.component.html',
  styles: [
  ]
})
export class VendorpaymentvouchersummaryComponent extends ParentFinMgtComponent implements OnInit {
  ...
  search(page: number = this.pageService.page, pageCount: number = this.pageService.pageCount) {
    ...
    this.apiService.getall(`report/${this.isSummary ? 'getVendorVoucherPaymentSummaryList' : 'getVendorVoucherPaymentDetailsList'}?page=${page}&pageCount=${pageCount}&isAllVendors = ${ this.isSummary }&custCode=${ this.codeControl.value }&from=${ this.utilService.getCommonDate(this.dateFrom) }&to=${ this.utilService.getCommonDate(this.dateTo) }`).subscribe(res => {
