...
export class RouteMasterComponent extends ParentFleetMgtComponent implements OnInit {
  @ViewChild(MatPaginator, { static: true }) paginator!: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort!: MatSort;
  displayedColumns: string[] = ['routeCode', 'routeName','routeNameAr','city','Actions'];
  ...
  get(): Array<any> {
    return [
      { id: 1, routeName: 'routeName-1', city: 'city-3' },
      { id: 1, routeName: 'routeName-2', city: 'city-3' },
      { id: 1, routeName: 'routeName-3', city: 'city-3' }
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('routeMaster', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  private openDialogManage(row: any) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdateRouteMasterComponent);
