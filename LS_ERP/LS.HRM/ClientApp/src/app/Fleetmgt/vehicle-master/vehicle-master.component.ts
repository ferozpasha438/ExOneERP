...
export class VehicleMasterComponent extends ParentFleetMgtComponent implements OnInit {
  @ViewChild(MatPaginator, { static: true }) paginator!: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort!: MatSort;
  displayedColumns: string[] = ['registrationNumber', 'vehicleCompany', 'vehicleType', 'brand', 'vehicleOwnerEnglish','vehicleOwnership',  'Actions'];
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('vehicleMaster', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  private openDialogManage(row: any) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdateVehicleMasterComponent);
  ...
  //private openDialogFuelEntryManage(row: any) {
  //  let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdateFuelEntryComponent);
  //  (dialogRef.componentInstance as any).row = row;
  ...
  //public editFuelEntry(row: any) {
  //  this.openDialogFuelEntryManage(row);
  //}
