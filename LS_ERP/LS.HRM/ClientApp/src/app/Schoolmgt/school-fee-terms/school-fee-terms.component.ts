...
@Component({
  selector: 'app-school-fee-terms',
  templateUrl: './school-fee-terms.component.html',
  styleUrls: []
})
export class SchoolFeeTermsComponent extends ParentSchoolMgtComponent implements OnInit{
  @ViewChild(MatPaginator, { static: true }) paginator!: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort!: MatSort;
  displayedColumns: string[] = ['termCode', 'termName', 'termNameInArabic',  'Actions'];
  ...
  get():Array<any>{
    return [
      { id: 1, termCode: 'Term -1', termName: 'Term -2', termName2:'Term -3'},
      { id: 1, termCode: 'Term -1', termName: 'Term -2', termName2:'Term -3'},
      { id: 1, termCode: 'Term -1', termName: 'Term -2', termName2:'Term -3'},
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('schoolFeeTerms', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  private openDialogManage(row: any) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdteSchoolFeeTermsComponent);
