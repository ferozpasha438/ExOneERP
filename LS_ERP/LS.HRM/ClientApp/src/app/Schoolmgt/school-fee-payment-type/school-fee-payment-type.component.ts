...
@Component({
  selector: 'app-school-fee-payment-type',
  templateUrl: './school-fee-payment-type.component.html',
  styleUrls: []
})
export class SchoolFeePaymentTypeComponent extends ParentSchoolMgtComponent implements OnInit{
  @ViewChild(MatPaginator, { static: true }) paginator!: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort!: MatSort;
  displayedColumns: string[] = ['paymentCode', 'paymentName', 'paymentNameInArabic',  'Actions'];
  ...
  get():Array<any>{
    return [
      {id:1,paymentCode:'Fee -1',paymentName:'Fee -2',paymentNameInArabic:'Fee -3'},     
      {id:1,paymentCode:'Fee -1',paymentName:'Fee -2',paymentNameInArabic:'Fee -3'},     
      {id:1,paymentCode:'Fee -1',paymentName:'Fee -2',paymentNameInArabic:'Fee -3'},     
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('studentPayType', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  private openDialogManage(row: any) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdateSchoolFeePaymentTypeComponent);
