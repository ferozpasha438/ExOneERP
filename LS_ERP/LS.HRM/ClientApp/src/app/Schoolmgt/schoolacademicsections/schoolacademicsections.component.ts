...
@Component({
  selector: 'app-schoolacademicsections',
  templateUrl: './schoolacademicsections.component.html',
  styleUrls: []
})
export class SchoolacademicsectionsComponent extends ParentSchoolMgtComponent implements OnInit {

  @ViewChild(MatPaginator, { static: true }) paginator!: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort!: MatSort;
  displayedColumns: string[] = ['sectionCode', 'sectionName', 'sectionNameInArabic', 'Actions'];
  ...
  get(): Array<any> {
    return [
      { id: 1, sectionCode: 'SOCIAL', sectionName: 'Social Studied', sectionNameInArabic: 'الدراسات الاجتماعية' },
      { id: 1, sectionCode: 'SOCIAL', sectionName: 'Social Studied', sectionNameInArabic: 'الدراسات الاجتماعية' },
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('schoolSections', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  private openDialogManage(row: any) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdateschoolacademicsectionsComponent);
