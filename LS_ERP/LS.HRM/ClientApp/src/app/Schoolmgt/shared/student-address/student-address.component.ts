...
export class StudentAddressComponent extends ParentSchoolMgtComponent implements OnInit {
  @ViewChild(MatPaginator, { static: true }) paginator!: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort!: MatSort;
  displayedColumns: string[] = ['buildingName','pAddress1', 'city', 'zipCode', 'country', 'phone1', 'mobile1', 'Actions'];
  ...
  constructor(private fb: FormBuilder, private http: HttpClient, private router: Router, private apiService: ApiService,
    ...
    public pageService: PaginationService, public dialogRef: MatDialogRef<StudentAddressComponent>) {
    super(authService);
  }
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    ...
    this.apiService.getPagination('SchoolStudentAddress/GetStudentAddressList', this.utilService.getStudentQueryString(this.stuAdmNum, page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  submit() {
    if (this.form.valid) {
      if (this.stuAddressId > 0)
        this.form.value['id'] = this.stuAddressId;
      this.form.value['stuAdmNum'] = this.stuAdmNum;
      this.apiService.post('SchoolStudentAddress', this.form.value)
        .subscribe(res => {
  ...
  public edit(item: any) {
    this.stuAddressId = item.id;
    if (item.id > 0) {
      this.form.patchValue({
        'buildingName': item.buildingName,
        'pAddress1': item.pAddress1,
        'city': item.city,
