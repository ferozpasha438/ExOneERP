...
@Component({
  selector: 'app-fee-structure-details',
  templateUrl: './fee-structure-details.component.html',
  styleUrls: []
})
export class FeeStructureDetailsComponent extends ParentSchoolMgtComponent implements OnInit {
  @ViewChild(MatPaginator, { static: true }) paginator!: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort!: MatSort;
  displayedColumns: string[] = ['structCode', 'feeStructureName', 'gradeCode','termCode', 'feeCode', 'feeTypeName', 'totalAmount', 'tax', 'netFee'];
  ...
  constructor(private fb: FormBuilder, private http: HttpClient, private router: Router, private apiService: ApiService,
    ...
    public pageService: PaginationService, public dialogRef: MatDialogRef<FeeStructureDetailsComponent>) {
    super(authService);
  }
  //get():Array<any>{
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('reports/FeeStructureDetailsReport', this.utilService.getQueryString(page, pageCount, query, orderBy, "", "", this.id)).subscribe(result => {
      this.totalItemsCount = 0;
