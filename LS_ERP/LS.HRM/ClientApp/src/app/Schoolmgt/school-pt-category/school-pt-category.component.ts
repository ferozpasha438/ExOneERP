...
@Component({
  selector: 'app-school-pt-category',
  templateUrl: './school-pt-category.component.html',
  styleUrls: []
})
export class SchoolPtCategoryComponent extends ParentSchoolMgtComponent implements OnInit {
  @ViewChild(MatPaginator, { static: true }) paginator!: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort!: MatSort;
  displayedColumns: string[] = ['ptCode', 'ptName', 'ptNameInArabic', 'Actions'];
  ...
  get(): Array<any> {
    return [
      { id: 1, petCode: 'SEM-1', petName: 'Semester 1', petName2: 'الفصل الدراسي 1' },
      { id: 1, petCode: 'SEM-1', petName: 'Semester 1', petName2: 'الفصل الدراسي 1' },
      { id: 1, petCode: 'SEM-1', petName: 'Semester 1', petName2: 'الفصل الدراسي 1' },
    ]
  }
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('schoolPETCategory', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  private openDialogManage(row: any) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdateSchoolPtCategoryComponent);
