...
@Component({
  selector: 'app-school-leaves-type',
  templateUrl: './school-leaves-type.component.html',
  styleUrls: []
})
export class SchoolLeavesTypeComponent extends ParentSchoolMgtComponent implements OnInit{
  @ViewChild(MatPaginator, { static: true }) paginator!: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort!: MatSort;
  displayedColumns: string[] = ['leaveCode', 'leaveName', 'leaveNameInArabic',  'Actions'];
  ...
  get():Array<any>{
    return [
      { id: 1, leaveCode: 'Leave -1', leaveName: 'Leave -1', leaveName2:'Leave -2'},
      { id: 1, leaveCode: 'Leave -1', leaveName: 'Leave -1', leaveName2:'Leave -2'},
      { id: 1, leaveCode: 'Leave -1', leaveName: 'Leave -1', leaveName2:'Leave -2'},
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('schoolStuLeaveType', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  private openDialogManage(row: any) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdteSchoolLeavesTypeComponent);
