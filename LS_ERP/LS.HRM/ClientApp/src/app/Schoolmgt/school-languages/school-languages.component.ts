...
@Component({
  selector: 'app-school-languages',
  templateUrl: './school-languages.component.html',
  styleUrls: []
})
export class SchoolLanguagesComponent extends ParentSchoolMgtComponent implements OnInit{
  @ViewChild(MatPaginator, { static: true }) paginator!: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort!: MatSort;
  displayedColumns: string[] = ['languageCode', 'languageName', 'languageNameInArabic',  'Actions'];
  ...
  get():Array<any>{
    return [
      { id: 1, langCode: 'Language -1', langName: 'Language -2', langName2:'Language -3'},
      { id: 1, langCode: 'Language -1', langName: 'Language -2', langName2:'Language -3'},
      { id: 1, langCode: 'Language -1', langName: 'Language -2', langName2:'Language -3'},
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('schoolLanguages', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  private openDialogManage(row: any) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdateschoolLanguagesComponent);
