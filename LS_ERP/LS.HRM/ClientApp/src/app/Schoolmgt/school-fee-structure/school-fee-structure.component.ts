...
@Component({
  selector: 'app-school-fee-structure',
  templateUrl: './school-fee-structure.component.html',
  styleUrls: []
})
export class SchoolFeeStructureComponent extends ParentSchoolMgtComponent implements OnInit {
  @ViewChild(MatPaginator, { static: true }) paginator!: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort!: MatSort;
  displayedColumns: string[] = ['feeStructureCode', 'feeStructureName', 'feeStructureNameArabic', 'branchCode', 'Actions'];
  ...
  get(): Array<any> {
    return [
      { id: 1, feeStructureCode: 'Fee -1', feeStructureName: 'Fee -2', feeStructureNameArabic: 'Fee -3', branchCode: 'Branch-1' },
      { id: 1, feeStructureCode: 'Fee -1', feeStructureName: 'Fee -2', feeStructureNameArabic: 'Fee -3', branchCode: 'Branch-1' },
      { id: 1, feeStructureCode: 'Fee -1', feeStructureName: 'Fee -2', feeStructureNameArabic: 'Fee -3', branchCode: 'Branch-1' },
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('schoolFeeStructure/getSysSchoolFeeStructureHeaderList', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  private openDialogManage(row: any) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdateSchoolFeeStructureComponent);
