...
@Component({
  selector: 'app-school-nationality',
  templateUrl: './school-nationality.component.html',
  styleUrls: []
})
export class SchoolNationalityComponent extends ParentSchoolMgtComponent implements OnInit {
  @ViewChild(MatPaginator, { static: true }) paginator!: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort!: MatSort;
  displayedColumns: string[] = ['nationalityCode', 'nationalityName', 'nationalityNameInArabic',  'Actions'];
  ...
  get():Array<any>{
    return [
      { id: 1, natCode: 'SEM-1', natName: 'Semester 1', natName2:'الفصل الدراسي 1'},
      { id: 1, natCode: 'SEM-1', natName: 'Semester 1', natName2:'الفصل الدراسي 1'},
      { id: 1, natCode: 'SEM-1', natName: 'Semester 1', natName2:'الفصل الدراسي 1'},
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('schoolNational', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  private openDialogManage(row: any) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdateSchoolNationalityComponent);
