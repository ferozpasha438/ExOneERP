...
namespace LS.API.MOB.Controllers
{
    ...
    public class BaseController : ApiControllerBase
    {
        ...
        public BaseController(IOptions<AppMobileSettingsJson> appSettings)
        {
        ...
        protected UserMobileIdentityDto UserInfo() => new UserMobileIdentityDto { UserId = UserId, EmployeeId = EmployeeId, CompanyId = CompanyId, BranchCode = BranchCode, BranchId = BranchId, Culture = Culture, SiteCode = SiteCode, ModuleCodes = "ADM,FI,FIN,INVT,PURC,SALE" }; //ConnectionString = GetConnectionString(),
        ...
        public long EmployeeId
        {
            get
            {
                if (User != null && User.Claims.Count() != 0 && User.Claims.FirstOrDefault(c => c.Type == "employeeId").Value != null)
                    return Convert.ToInt32(User.Claims.FirstOrDefault(c => c.Type == "employeeId").Value);
                return 0;
            }
        }
        ...
        public string SiteCode
        {
            get
            {
                if (User != null && User.Claims.Count() != 0 && User.Claims.FirstOrDefault(c => c.Type == "sitecode").Value != null)
                    return Convert.ToString(User.Claims.FirstOrDefault(c => c.Type == "sitecode").Value);
                return string.Empty;
            }
        }
    ...
    public class FileBaseController : BaseController
    {
        ...
        private readonly IOptions<AppMobileSettingsJson> _appSettings;
        public FileBaseController(IOptions<AppMobileSettingsJson> appSettings, IWebHostEnvironment env) : base(appSettings)
        {
            _env = env;
            _appSettings = appSettings;
        }
